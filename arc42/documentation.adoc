...
=== UI Informationen (Jan)
Das anzeigen Wichtiger funktionen für den Nutzer bei der Abstimmung wurde Implementiert . Dies ist wichtig, da der Nutzer dann sieht wie die Regeln einer Abstimmung sind.
...
=== Tests (Julia, Kathi)
Am Vortag hat Julia noch Kleinigkeiten an den Kommentaren geändert, sodass nun keine leeren Kommentare
abgeschickt werden können und der Name des Kommentarautors direkt auf den Benutzernamen gesetzt wird,
wenn kein Pseudonym übergeben wird. An diesem Tag wurden dann noch von Kathi und Julia fehlende Tests für
das filehandling-Package geschrieben (für die Klassen, die das Format prüfen bzw. ändern beim Import bzw. Export).
Außerdem wurden diverse pull requests bearbeitet, um auch die restlichen Änderungen in den master
zu bringen. Zudem hat Julia noch Tests für den UmfragenAbstimmungsController geschrieben.
