...
Ziel war es eine Funktion implenentieren , die es Ermöglicht bei der Erstellung einer Terminfindung
einzustellen ob ein Nutzer seine Abstimmung bis zur Frist bearbeiten darf.
Html und Logik wurden geschrieben.

=== Umfragen (Anton)
Angefangen thymeleaf in die Abstimmungs- und Ergebnisviews für Umfragen einzubinden.
...
Den Selector für Gruppenauswahl auf der Umfragenübersicht gefixt.
Versucht mit Ingrid die Frist der Abstimmunegn so festzulegen, dass die Frist erst nach den möglichen Terminen liegen muss, es hat aber nicht funktioniert.
...
Es wurden Tests den Controller der Terminübersicht geschrieben.
...
erwartet. Dies macht Sinn, da besonders wenn mehrere Zeilen aus einer Tabelle gelöscht werden und mittendrin ein Fehler auftritt, dass dann nicht nur die Hälfte der Daten gelöscht werden.
...
Außerdem wurde das erstellen eines Kommentars auf der Abstimmungsseite und der
Ergebnisseite ermöglicht.
...
=== UI Fixes (Anton)
Die Auswahl der Gruppen und den Übersichten bearbeitet, so dass sie nicht überläuft und man bei großer Anzahl von Gruppen in der Liste nicht durch die ganze scrollen muss.
Darstellung von Titeln und Zeiten in Übersichten korrigiert.

=== Kommentare (Julia, Kathi, Anton)
Wir haben alle Komponenten der Kommentare für Umfragen geschrieben und in master gemergt.

=== Umfrage Abstimmung und Ergebnis Seite(Thymeleaf, Services) (Jan, Thomas, Anton)
Das Thymeleaf für die Abstimmungs und Ergebnisseite geschrieben.
Services angepasst. Die entstandenen Probleme wurden alle von Thomas gefunden und korrigiert.
...
CSV-Datei exportiert werden. Das Format entspricht dem für den Upload, sodass beim Ertsellen einer neuen Terminfindung
